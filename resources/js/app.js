...
var languagesPath = "./languages" //where we store our languages
...
window.onload = async function() {
    ...
    //for the save as file dialog
    saveAsInput.addEventListener('change', (event) => {
      const file = event.target.files[0];
      curFile.path = file.path;
...
async function loadLanguages(directoryPath){
  ...
  files.forEach(async (file) => {
    ...
    //access our files
    let data = await window.Neutralino.filesystem.readFile('./myFile.txt');
